...
/**
 * Add route to rocket webserver that calculates mandelbrot fractal and return it as byte array (8-bit grayscale)
 */
...
fn mandelbrot(min_c_re :f32, min_c_im :f32, max_c_re :f32, max_c_im :f32, xsize :u32 ,ysize :u32, inf_n :u32) -> Vec<u8> {
	...
	for y in 0..ysize {
		for x in 0..xsize {
			let mut zx = 0.0;
			let mut zy = 0.0;
			let c_x = x as f32 * (max_c_re - min_c_re) / xsize as f32 + min_c_re;
			let c_y = y as f32 * (max_c_im - min_c_im) / ysize as f32 + min_c_im;
            let mut iter = inf_n;
